...
class SchemaGrammarTest extends TestCase
{
    ...
    #[Test]
    public function it_names_the_inline_identity_primary_key_from_the_template()
    {
        $connection = $this->makeConnection([
            'server_version' => '5.0.3',
            'index_names' => ['primary' => 'PK_{table}'],
        ]);

        $sql = $this->firebirdCreateSql($connection, 'usuario', function ($table) {
            $table->id();
            $table->string('nome');
        });

        $this->assertStringContainsString(
            'GENERATED BY DEFAULT AS IDENTITY CONSTRAINT "PK_usuario" PRIMARY KEY',
            $sql
        );
    }

    #[Test]
    public function it_names_the_inline_legacy_primary_key_from_the_template()
    {
        $connection = $this->makeConnection([
        ...
        $this->assertStringContainsString('"id" INTEGER CONSTRAINT "PK_usuario" PRIMARY KEY', $sql);
    }

    #[Test]
    public function it_keeps_the_inline_primary_key_unnamed_without_a_template()
    {
        $connection = $this->makeConnection(['server_version' => '5.0.3']);
        ...
        $this->assertStringContainsString('GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY', $sql);
